...
import type { CreditOperation, CreditOperationsResponse } from "../types.js";
...
export interface CreditOperationsState {
...
export function useCreditOperations(
  options: UseCreditOperationsOptions = {},
  ...
  const requestPage = useCallback(
    async (
      cursor: string,
      signal?: AbortSignal,
    ): Promise<CreditOperationsResponse> => {
      ...
      const response = await client.creditFetch(
        `/api/v1/customers/${encodeURIComponent(customerId)}/credit-operations?${search.toString()}`,
        { signal },
      );
      if (!response.ok)
        throw new Error(
          `credit operations request failed with HTTP ${response.status}`,
        );
      return response.json() as Promise<CreditOperationsResponse>;
    },
    [client, creditSystemCode, customerId, pageSize],
  );
