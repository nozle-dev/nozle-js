...
export interface ProductCreditBalanceProps {
...
export function ProductCreditBalance({
  creditSystemCode,
  customerId,
  maximumFractionDigits = 12,
  className,
  style,
}: ProductCreditBalanceProps) {
  ...
  if (isLoading) {
    return (
      <div
        role="status"
        aria-label="Loading product credit balance"
        className={className}
        style={{
          width: "8rem",
          height: "2rem",
  ...
  if (error || !data) {
    return (
      ...
      >
        Failed to load product credit balance
      </span>
    );
  }
